...
int decode (char *rank, char *suit) // преобразует текст в номера карты
 {
 	...
 	if (n==-1) return (-1);
 	if (strcmp(suit,SUIT_HEARTS)==0) r=n; // масти "добавляют" соответствующее "смещение" в число, кратное 9 (черви 0-8, бубны 9-17 и т.д.)
...
void draw(void)
 {
 	...
 	for(i=0;i<40;++i) printf("\xc4");
 	printf("\xd9\n");
 	for(i=0;i<=_uhand/15;++i) // 15 карт в строке (одна карта - 5 симоволов:|10*|, в консоли обычно 80 столбцов, 80/5=16, но мы делаем с запасом)
 	 {
 	 	for(n=0;n<3;++n) // аккуратно выводим каждую "строчку" карт по 15 штук в каждой в три "прохода"
 	 	 {
 	   	for(j=i*15;(j<_uhand)&&(j<i*15+15);++j) // сложное условие для проверки - цикл длится пока оба условия истинны. Это позволяет "отрезать" цикл до 15 элеменов (правая часть)
...
// 	 dumpv(&stack[0],_stack," CLEAR STACK DUMP "); // раскомментировать чтобы посмотреть колоду до перемешивания
			n=rand()%500+500; // перемешиваем колоду от 500 до 1000 раз. Одно перемешивание подразумевает перестановку двух РАЗНЫХ карт в массиве между собой.
			for (i=0; i<n; ++i)
			 {
			 	 a=rand()%36; // случайная карта 
			 	 b=rand()%36; // вторая случайная карта
			 	 if (a==b) // если вышло так, что обе карты оказались одной и той же картой, нужно повторить заново (вероятность порядка (1/36)*(1/36)=0.08%, но тем не менее, ведь это, возможно, плохой генератор случайных чисел)
						 {
...
// 	 dumpv(&stack[0],_stack," MIXED STACK DUMP "); //  можно подсмотреть перемешанную колоду
 	 ...
 	 for(i=35;i>0;--i) stack[i]=stack[i-1]; // содержимое колоды "приподнимается" на одну позицию, чтобы на дно (i=0) положить карту-козырь.
 	 ...
 	 while (1==1) // игра в бесконечном цикле
 	  {
 	  	...
 	  	if ((_stack==0)&&((_uhand==0)||(_chand==0))) break; // игра закончена если в колоде нет карт и при этом их нет на руках по крайней мере у одного игрока.
 	  	
			  draw();
   	 if (whonext==0) // ходит человек
   	  {
 	    	n=0;
 	    	printf("\n\n(%s|%s|%s|%s|%s|%s|%s|%s|%s) ",RANK_6_LONG,RANK_7_LONG,RANK_8_LONG,RANK_9_LONG,RANK_10_LONG,RANK_J_LONG,RANK_Q_LONG,RANK_K_LONG,RANK_A_LONG);
 	    	printf("(%s|%s|%s|%s)\n\tor\t",SUIT_HEARTS,SUIT_DIAMONDS,SUIT_CLUBS,SUIT_SPADES);
 	    	printf("(%s|%s|%s|%s|%s|%s|%s|%s|%s) ",RANK_6_SHORT,RANK_7_SHORT,RANK_8_SHORT,RANK_9_SHORT,RANK_10_SHORT,RANK_J_SHORT,RANK_Q_SHORT,RANK_K_SHORT,RANK_A_SHORT);
       ...
       if ((strcmp(rank,"i")==0)&&(strcmp(suit,"take")==0)) // пользователь решил взять карты на столе (набрал "i take")
        {
        	for (i=0; i<_chand; ++i) // ПК перебирает свои карты в поисках того, что можно подбросить вдогонку
        	 {
        	 	...
        	 	for (n=0; n<_table; ++n)
        	 	 {
        	 	 	if (chand[i]%9 != table[n]%9) continue; // ПК нельзя подкидывать карты с достоинствами, которых нет на столе
        	 	 	...
        	 	 	a=1; b=i;
				      			...
				      			for (n=b; n<=_chand; ++n)
    						  	 {
    							   	chand[n]=chand[n+1]; // карты в руке перемещаются на 1 позицию "вниз", заполнив "пустое" место uhand[a]
  						    		}
   	  	...
   	  	if (n==-1)
   	  	 {
   	  	 	...
   	  	 	continue;
							...
							if ((_table>0)&&(_table%2==0)) // если карты лежат на столе, нельзя ходить с карты прочего достоинства
							 {
							 	a=0;
							 	for (i=0; i<_table; ++i)
							 	 {
							...
							if (_table%2==1) // пользователь отбивается - его ход и карт на столе нечётное количество
							 {
							 	if (table[_table-1]/9 == uhand[a]/9) // бьётся одинаковой мастью
							 	 { 
							 	   if (table[_table-1]%9 < uhand[a]%9)
							 	    { // карта той же масти у пользователя подходит для отбивания
					       		table[_table]=uhand[a]; // кладём карту на стол
       							_table++; // на столе +1 карта
      	 						for (i=a; i<=_uhand; ++i)
       							 {
       							 	uhand[i]=uhand[i+1]; // карты в руке перемещаются на 1 позицию "вниз", заполнив "пустое" место uhand[a]
       								}
       							...
       							continue;
												 ...
												 { // карта той же мастью но младше - отбиваться нельзя!
					 	 	...
					 	 	if (_table%2==1) // если на столе нечётное число карт то я отбиваюсь...
					 	 	 {
					 	 	 	a=-1;
					 	 	 	b=10;
					 	 	 	for(i=0; i<_chand; ++i) // ищем той же масти, что последняя карта на столе. Верхняя (небитая "верхняя" карта на столе - table[_table-1])
					 	 	 	 {
					 	 	 	 	if (chand[i]/9 == table[_table-1]/9) // моя карта той же масти, что и на столе
					 	 	 	 	 {
					 	 	 	 	 	if (chand[i]%9 < table[_table-1]%9) continue; // эта карта младше той, что на столе, поехали смотреть следующую
					 	 	 	 	 	if ((chand[i]%9 < b)&&(chand[i]%9 > table[_table-1]%9)) // мы нашли карту старше, чем на столе, но в цикле найдём из них самую маленькую.
					 	 	 	 	 	 {
					 	 	 	 	 	 	b=chand[i]%9;
					 	 	 	 	 	 	a=i;
																}
											...
											if (a==-1) // карты той же масти не нашли, давайте искать козыри
											 {
											 	b=10;
  					 	 	 	for(i=0; i<_chand; ++i) // ищем козыри. Верхняя (небитая "верхняя" карта на столе - table[_table-1])
		  			 	 	 	 {
...
//			 	 	   	 	 	if ((chand[i]%9 < b)&&(chand[i]%9 > table[_table-1]%9)) // мы нашли карту-козырь, но в цикле найдём из них самую маленькую.
			 	 	   	 	 	if (chand[i]%9 < b) // мы нашли карту-козырь, но в цикле найдём из них самую маленькую.
					 	 	 	 	 	 {
    										  	if (chand[i]/9 == table[_table-1]/9)
    										  	 if (chand[i]%9 < table[_table-1]%9) continue; // если на столе козырь его нужно отбивать как минимум, бОльшим козырем, а не меньшим.
  			 	 	 	 	 	 	b=chand[i]%9;
		  	 	 	 	 	 	 	a=i;
				  										}
										  		}
      							...
      							for (i=a; i<=_chand; ++i)
      							 {
      							 	chand[i]=chand[i+1]; // карты в руке перемещаются на 1 позицию "вниз", заполнив "пустое" место uhand[a]
      								}
      							_chand--; // в руке на карту меньше
      							whonext=(whonext==1)?0:1;
      							continue;
											 ...
											 {
											 	for (i=_uhand; i>=0; i--) // отдавать карты ПК будем в цикле. Цикл сможет продолжиться столько раз, сколько сейчас карт в руке пользователя.
											 	 {
  											 	draw();
  											 	printf("BERU!!!");
   	  	        ...
   	  	        n=decode(rank, suit);
   	  	        if ((strcmp(rank,"end")==0)&&(strcmp(suit,"drop")==0)) // end drop, т.е. больше чем подкинули не подкидываем
   	  	         {
   	  	         	for (i=0; i<_cadd; ++i) // перекиываем компьютеру в руку из массива для подкидывания
   	  	         	 {
   	  	         	 	...
   	  	         	 	_chand++;
																	...
																	for (i=0; i<36; i++) cadd[i]=-1; // на всякий случай почистим массив для подкидвания
   	  	        ...
   	  	        if (n==-1) // decode() вернуло -1. Это означает что мы ничего не смогли разобрать в вводе пользователя.
   	  	         {
           	  	 	printf("\nI don't understand you!!!\n");
           	  	 	sleep(3);
   	  	         	i++;
									 	 	 	 	...
									 	 	 	 	if (table[c]%9 == chand[i]%9)
									 	 	 	 	 {
															...
															if (a==1)
															 { // ПК нашёл некозырную карту для подброса
															 	b=chand[i]%9;
															 	n=i;
																}
														}
